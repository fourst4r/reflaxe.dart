...
class TestCases {
	...
	@:generic static function eq<T>(expected:T, actual:T, ?p:PosInfos) {
		...
		if (expected != actual) {
			numFailures++;
			Sys.println('${p?.lineNumber}: Failed!');
		}
	}
	...
	static var trueValue:Bool = true;
	static var falseValue:Bool = false;
	static var staticVar:Int = 0;
	static var staticNullVar:Null<Int> = null;
	...
	static function testNativeArray() {
		var asdf:Vector<Int> = new Vector(10);
		trace(asdf);
		var a:Vector<Vector<Int>> = new Vector(1);
		a[0] = new Vector(1);
		// eq(0, a[0][0]);
		eq(12, a[0][0] = 12);
		eq(12, a[0][0]);
	...
	var localVar:Int = 0;
	var localNullVar:Null<Int> = null;
